...
def _run_pydatalog_facts(threat: str, verification: float) -> List[SymbolicFinding]:
    ...
    try:
        ...
        if threat in {"HIGH", "CRITICAL"}:
            +is_high_risk("current_assessment")  # noqa: F821
        if verification < 0.5:
            +lacks_verification("current_assessment")  # noqa: F821

        # Define logical rule: it's a contradiction if it's high risk but lacks verification
        contradiction(X) <= is_high_risk(X) & lacks_verification(X)  # noqa: F821

        # Query the logic engine
        if contradiction("current_assessment"):  # noqa: F821
            findings.append(
                SymbolicFinding(
