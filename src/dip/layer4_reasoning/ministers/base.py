...
from dip.nextgen.structured_llm import structured_acompletion
...
class BaseMinister(ABC):
    ...
    def _heuristic_proposal(self, ctx: StateContext) -> MinisterHypothesisOutput:
        ...
        if ctx.current_signals:
            signal_pressure = sum(
                max(0.0, min(1.0, sig.confidence * max(sig.intensity, 0.25)))
                for sig in ctx.current_signals
            ) / len(ctx.current_signals)

        match_score = len(matched) / max(len(predicted), 1)
        confidence = max(match_score, min(1.0, 0.45 * signal_pressure + 0.35 * sre_score))
        ...
        return MinisterHypothesisOutput(
            ...
            rationale="Deterministic heuristic proposal from observed signals and SRE pressure.",
        )
