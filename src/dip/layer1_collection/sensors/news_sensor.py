...
class NewsSensor:
    ...
    async def _fetch_ddg(self, country: str) -> List[RawObservation]:
        """Fetch news from DuckDuckGo News search."""
        ...
        try:
            ...
            results = list(ddgs.news(query, max_results=15, region="wt-wt"))

            for res in results:
                title = res.get("title", "")
                url = res.get("url", "")
                pub_date = res.get("date", datetime.now(timezone.utc).isoformat())
                body = res.get("body", "")

                matched_cameo = self._classify_signal(f"{title} {body}")
            ...
            logger.info(f"DuckDuckGo: fetched {len(observations)} news signals for {country}")

        except Exception as e:
            logger.warning(f"DuckDuckGo News fetch failed: {e}")
