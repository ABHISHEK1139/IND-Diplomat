...
STANDALONE_SCRIPTS = [
    "tests/run_fallback_test.py",
    "tests/manual/manual_test_anomaly.py",
    "tests/manual/run_real_world_test.py",
    "tests/test_drill.py",
    "tests/test_system.py",
    "tests/system_bootstrap/test_internet.py",
    "tests/system_bootstrap/test_ocr.py",
    "tests/system_bootstrap/test_rag.py",
]
...
def main() -> int:
    ...
    run_dir = PROJECT_ROOT / ".local" / "runtime" / "test_runs" / stamp
    ...
    try:
        summary["pytest"] = _run_command(
            [sys.executable, "-m", "pytest", "tests", "-vv", "--maxfail=0", f"--junitxml={run_dir / 'pytest_junit.xml'}"],
            cwd=PROJECT_ROOT,
            env=env,
